...
@section('title', '貸与先編集 | Argus')

@section('content')
<div class="mx-auto max-w-3xl">
    <h2 class="mb-6 text-2xl font-semibold">貸与先編集</h2>
    ...
    <form method="POST" action="{{ route('masters.assignees.update', ['assigneeId' => $assignee->id]) }}" class="space-y-4 rounded border border-slate-200 bg-white p-6">
        @csrf
        @method('PUT')

        <div>
            <label for="name" class="mb-1 block text-sm">貸与先名 *</label>
            <input id="name" name="name" value="{{ old('name', $assignee->name) }}" class="w-full rounded border border-slate-300 px-3 py-2">
        ...
        <div>
            ...
            <select id="assignee_type" name="assignee_type" class="w-full rounded border border-slate-300 px-3 py-2">
                @foreach ($assigneeTypes as $type)
                    <option value="{{ $type }}" @selected(old('assignee_type', $assignee->assignee_type) === $type)>{{ $type }}</option>
                @endforeach
            </select>
        ...
        <div class="flex gap-3">
            <button class="rounded bg-slate-800 px-4 py-2 text-white">更新</button>
