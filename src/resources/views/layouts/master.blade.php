...
<div class="min-h-screen md:flex">
    <aside class="w-full border-b border-slate-200 bg-white md:min-h-screen md:w-64 md:border-b-0 md:border-r">
        <div class="border-b border-slate-200 px-5 py-4">
            ...
            <h1 class="text-lg font-semibold">メニュー</h1>
        </div>

        <nav class="space-y-1 p-3 text-sm">
            <a href="{{ route('dashboard.index') }}" class="block rounded px-3 py-2 {{ ($currentMaster ?? '') === 'dashboard' ? 'bg-slate-900 text-white' : 'text-slate-700 hover:bg-slate-100' }}">ダッシュボード</a>
        </nav>
    </aside>
